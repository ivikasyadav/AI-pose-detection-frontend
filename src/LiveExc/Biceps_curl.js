...
const App = () => {
    ...
    const curlState = useRef("DOWN"); // Track curl state without re-renders
    const repsRef = useRef(0); // Track reps directly to avoid async state issues
        ...
        if (videoRef.current.readyState === 4) {
            ...
            if (poses.length > 0) {
                ...
                if (leftShoulder && leftElbow && leftWrist) {
                    ...
                    console.log("Elbow Angle:", angle, "Curl State:", curlState.current);

                    // Improved rep-counting logic with debounce
                    if (curlState.current === "DOWN" && angle < 35) {
                        curlState.current = "UP"; // Arm is fully curled up
                        console.log("State Changed: UP");
                    } else if (curlState.current === "UP" && angle > 165) {
                        // Ensure reps are only counted once per cycle
                        if (Date.now() - lastRepTime.current > 1000) { // 1 sec debounce
                            repsRef.current += 1;
                            setReps(repsRef.current);
                            curlState.current = "DOWN"; // Reset state
